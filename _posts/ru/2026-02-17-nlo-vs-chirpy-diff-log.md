...
permalink: /posts/ru/nlo-vs-chirpy-diff-log/
...
![Логотип NLO](assets/img/logo_nlo.png)
Этот пост фиксирует **конкретные технические отличия** от оригинальной темы. Пункт добавляется, когда он реализован и проверен.
...
Новые отличия попадают в этот пост по мере реализации, сохраняя список прозрачным и проверяемым.
