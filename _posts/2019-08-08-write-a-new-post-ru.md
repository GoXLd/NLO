...
Вам нужно заполнить [Front Matter](https://jekyllrb.com/docs/front-matter/) в начале поста:
...
Информацию об авторе обычно не нужно задавать в _Front Matter_: по умолчанию она берется из `social.name` и первой записи `social.links` в конфигурации. При необходимости это можно переопределить:
...
По умолчанию оглавление (TOC) отображается в правой панели поста. Чтобы отключить его глобально, установите `toc: false` в `_config.yml`{: .filepath}. Чтобы отключить TOC только для одного поста, добавьте в его [Front Matter](https://jekyllrb.com/docs/front-matter/) следующее:
...
- Чтобы указать префикс пути к ресурсу для текущей записи/диапазона страниц, установите `media_subpath` во _Front Matter_ публикации:
